...
const FocusPage = React.lazy(() => import('./components/FocusPage').then(m => ({ default: m.FocusPage })));
const ProfileShowcase = React.lazy(() => import('./components/ProfileShowcase').then(m => ({ default: m.ProfileShowcase })));
const SubjectMapAdvanced = React.lazy(() => import('./components/SubjectMapAdvanced').then(m => ({ default: m.SubjectMapAdvanced })));
const ExamSimulator = React.lazy(() => import('./components/ExamSimulator').then(m => ({ default: m.ExamSimulator })));
const GraveyardPanel = React.lazy(() => import('./components/GraveyardPanel').then(m => ({ default: m.GraveyardPanel })));
const CoachScreen = React.lazy(() => import('./components/coach/CoachScreen').then(m => ({ default: m.CoachScreen })));
const StorePage = React.lazy(() => import('./components/store/StorePage').then(m => ({ default: m.StorePage })));
const LogHistory = React.lazy(() => import('./components/LogHistory').then(m => ({ default: m.LogHistory })));
const BentoDashboard = React.lazy(() => import('./components/dashboard/BentoDashboard').then(m => ({ default: m.BentoDashboard })));
const PublisherAnalytics = React.lazy(() => import('./components/PublisherAnalytics').then(m => ({ default: m.PublisherAnalytics })));
...
function SpotifyCallback() {
  ...
  const setSpotifyToken = useAppStore(s => s.setSpotifyToken);
  ...
  }, [navigate, toast, setSpotifyToken]);
...
export default function App() {
  ...
  // HOOKS-FIX: Must be called unconditionally BEFORE any early returns
  const viewingProfileUid = useAppStore(s => s.viewingProfileUid);
  ...
  const syncStatus = isOnline ? (pendingCount > 0 ? 'pending' : 'synced') : 'offline';
  const forceSync = async (_force?: boolean) => {
    triggerManualSync();
    await replayQueue();
  };
            ...
            <div className="fixed inset-0 z-[250] flex items-center justify-center p-4">
              ...
              >
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
                  <ProfileShowcase isPublic targetUid={viewingProfileUid!} />
                </div>
              </motion.div>
            </div>
          )}
