    ...
    if (this.state.hasError) {
      return (
        <div className="fixed inset-0 bg-[#0A0A0A] flex flex-col items-center justify-center p-8 text-center">
          <h1 className="text-4xl font-serif italic text-red-500 mb-4 animate-pulse">💥TAKLAYA GELDİN!💥</h1>
          <p className="text-zinc-400 font-mono text-sm max-w-md">Kritik bir modül çöktü. Boho'nun kalbi yerinden oynadı. Devreleri tamir edip en güncel haliyle başlatmak için:</p>
          <button
            onClick={() => hardReset()}
          ...
          >
            Bohoyu Tersten Görmek İçin Tıklayın
          </button>
        </div>
      );
    }
