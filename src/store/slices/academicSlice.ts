...
export const createAcademicSlice: StateCreator<AppState, [], [], AcademicSlice> = (set, get) => ({
  ...
  addLog: (log) => {
    ...
    if (authUser?.uid) {
      setDocWithOfflineQueue(doc(db, 'users', authUser.uid, 'logs', logWithId.id), cleanForFirestore(logWithId)).catch(console.error);
      setDocWithOfflineQueue(doc(db, 'users', authUser.uid), cleanForFirestore({ streakDays: newStreak }), { merge: true }).catch(console.error);
  ...
  removeLog: (id) => {
    ...
    if (authUser?.uid) {
      deleteDocWithOfflineQueue(doc(db, 'users', authUser.uid, 'logs', id)).catch(console.error);
      setDocWithOfflineQueue(doc(db, 'users', authUser.uid), cleanForFirestore({ streakDays: newStreak }), { merge: true }).catch(console.error);
    }
    detectAndSetHabits();
  },
  ...
  updateLog: (id, updates) => {
    ...
    if (authUser?.uid) {
      ...
      if (updated) setDocWithOfflineQueue(doc(db, 'users', authUser.uid, 'logs', id), cleanForFirestore(updated)).catch(console.error);
      setDocWithOfflineQueue(doc(db, 'users', authUser.uid), cleanForFirestore({ streakDays: newStreak }), { merge: true }).catch(console.error);
    }
    detectAndSetHabits();
  },
  ...
  addExam: (exam) => {
    ...
    if (authUser?.uid) {
      setDocWithOfflineQueue(doc(db, 'users', authUser.uid, 'exams', normalizedExam.id), cleanForFirestore(normalizedExam)).catch(console.error);
    }
  },
  ...
  removeExam: (id) => {
    ...
    if (authUser?.uid) {
      deleteDocWithOfflineQueue(doc(db, 'users', authUser.uid, 'exams', id)).catch(console.error);
    }
  },
  ...
  updateExam: (id, updates) => {
    ...
    if (authUser?.uid) {
      ...
      if (updated) setDocWithOfflineQueue(doc(db, 'users', authUser.uid, 'exams', id), cleanForFirestore(updated)).catch(console.error);
    }
  },
  ...
  addFailedQuestion: (input) => {
    ...
    if (authUser?.uid) {
      setDocWithOfflineQueue(doc(db, 'users', authUser.uid, 'failedQuestions', newQ.id), cleanForFirestore(newQ)).catch(console.error);
    }
  },
  ...
  solveFailedQuestion: (id) => {
    ...
    if (authUser?.uid) {
      ...
      if (updated) setDocWithOfflineQueue(doc(db, 'users', authUser.uid, 'failedQuestions', id), cleanForFirestore(updated)).catch(console.error);
    }
  },
  ...
  removeFailedQuestion: (id) => {
    ...
    set({ failedQuestions: newList });
    if (authUser?.uid) {
      deleteDocWithOfflineQueue(doc(db, 'users', authUser.uid, 'failedQuestions', id)).catch(console.error);
    }
  },
  ...
  addAgendaEntry: (entry) => {
    ...
    if (authUser?.uid) {
      setDocWithOfflineQueue(doc(db, 'users', authUser.uid, 'agendaEntries', entry.id), cleanForFirestore(entry)).catch(console.error);
    }
  },
  ...
  updateAgendaEntry: (id, updates) => {
    ...
    if (authUser?.uid) {
      ...
      if (updated) setDocWithOfflineQueue(doc(db, 'users', authUser.uid, 'agendaEntries', id), cleanForFirestore(updated)).catch(console.error);
    }
  },
  ...
  removeAgendaEntry: (id) => {
    ...
    set({ agendaEntries: newEntries });
    if (authUser?.uid) {
      deleteDocWithOfflineQueue(doc(db, 'users', authUser.uid, 'agendaEntries', id)).catch(console.error);
    }
  },
  ...
  addFocusSession: (record) => {
    ...
    if (authUser?.uid) {
      setDocWithOfflineQueue(doc(db, 'users', authUser.uid, 'focusSessions', record.id), cleanForFirestore(record)).catch(console.error);
    }
  },
  ...
  addFlashcard: (card) => {
    ...
    if (authUser?.uid) {
      setDocWithOfflineQueue(doc(db, 'users', authUser.uid, 'flashcards', card.id), cleanForFirestore(card)).catch(console.error);
    }
  },
  ...
  updateFlashcard: (id, updates) => {
    ...
    if (authUser?.uid) {
      ...
      if (updated) setDocWithOfflineQueue(doc(db, 'users', authUser.uid, 'flashcards', id), cleanForFirestore(updated)).catch(console.error);
    }
  },
