...
export function SubjectMasterySunburst() {
  ...
  const tytData = useMemo(() => {
    ...
    // STABLE-HOOKS-FIX: Do NOT filter — Recharts v3 uses hooks per data entry,
    // so array length must stay constant between renders
    return [
  ...
  const aytData = useMemo(() => {
    ...
    // STABLE-HOOKS-FIX: Use ALL track subjects as keys so array length stays constant
    const trackSubsOrdered = trackSubs;
    ...
    // Pre-populate all track subjects so array length is always the same
    trackSubsOrdered.forEach((sub, idx) => {
      groups[sub] = { mastered: 0, total: 0, fill: colors[idx % colors.length] };
    });
    filtered.forEach((s) => {
      if (groups[s.subject]) {
    ...
    return trackSubsOrdered.map(name => {
      const g = groups[name];
      return {
        name,
