...
export function BentoDashboard() {
  ...
  const todayHours = useMemo(() => (todayLogs.reduce((acc, log) => acc + (log?.avgTime || 0), 0) / 60).toFixed(1), [todayLogs]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <BentoStatCard title="Kritik Sorunlar" value={(logs || []).filter(l => (l?.wrong || 0) > (l?.correct || 0)).length.toString()} unit="Sorunlu" icon={<AlertTriangle className="text-orange-500" />} />
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {lastCoachDirective && (
            <div className="px-3 py-1 bg-white/5 rounded-full border border-white/5 text-[9px] font-black tracking-widest text-zinc-500 uppercase">
              {lastCoachDirective.tasks?.filter(t => t.status === 'completed').length || 0}/{lastCoachDirective.tasks?.length || 0} Tamamlandı
            </div>
          )}
        </div>
