...
const TopicCard = React.memo(function TopicCard({ sub, logs, onStatusChange, onNotesChange, searchQuery }: TopicCardProps) {
        ...
        <div className="flex items-center gap-2 shrink-0">
          {stats && (
            <div className="flex items-center gap-1.5">
              ...
              {/* Memory / Forgetting Curve Indicator */}
              <div 
                className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded border text-[9px] font-bold uppercase tracking-tighter
                  ${(() => {
                    const daysAgo = Math.floor((Date.now() - new Date(stats.lastLogDate).getTime()) / (1000 * 60 * 60 * 24));
                    if (daysAgo <= 3) return 'text-emerald-400 border-emerald-700/20 bg-emerald-500/5';
                    if (daysAgo <= 7) return 'text-amber-400 border-amber-700/20 bg-amber-500/5';
                    return 'text-rose-400 border-rose-700/20 bg-rose-500/5';
                  })()}
                `}
                title={`Son çalışma: ${stats.lastDate}`}
              >
                <Flame size={10} className="fill-current" />
                {(() => {
                  const daysAgo = Math.floor((Date.now() - new Date(stats.lastLogDate).getTime()) / (1000 * 60 * 60 * 24));
                  if (daysAgo <= 3) return 'Taze';
...
const SubjectGroup = React.memo(function SubjectGroup({
...
export function SubjectMapAdvanced() {
  ...
  const activeSubjects = activePanel === 'TYT' ? tytSubjects : trackAYT;
  ...
  const grouped = useMemo(() => {
    ...
    activeSubjects.forEach((sub, idx) => {
      ...
      const origIdx = activePanel === 'TYT'
        ? idx
        : aytSubjects.findIndex(a => a.name === sub.name && a.subject === sub.subject);
      if (!map.has(sub.subject)) map.set(sub.subject, []);
      map.get(sub.subject)!.push({ ...sub, originalIndex: origIdx });
    });
    return map;
  }, [activeSubjects, activePanel, aytSubjects]);
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex bg-zinc-900/80 border border-zinc-800 rounded-2xl p-1.5 gap-2">
        {(['TYT', 'AYT'] as const).map(panel => {
          const subs = panel === 'TYT' ? tytSubjects : trackAYT;
