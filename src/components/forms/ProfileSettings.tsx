...
export function ProfileSettings({ onSubmit, initialData, isEditMode = false }: ProfileSettingsProps) {
  ...
  const [coachPersonality, setCoachPersonality] = useState(initialData?.coachPersonality ?? 'enforcer');
...
function EditModeForm(props: {
  ...
  return <div style={{ fontFamily: "'Inter',system-ui,sans-serif" }}>
    <style>{`
      ...
      .em-input{width:100%;background:var(--color-surface);border:1px solid var(--color-border);border-radius:12px;padding:12px 16px;font-size:14px;color:var(--color-ink);outline:none;box-sizing:border-box}
      .em-input:focus{border-color:var(--color-accent)}
      .em-section{margin-bottom:24px}
      .em-title{font-size:11px;font-weight:700;letter-spacing:.15em;text-transform:uppercase;color:var(--color-accent);padding-bottom:12px;border-bottom:1px solid var(--color-border-subtle);margin-bottom:16px}
      .em-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}
      .em-pill{padding:8px 14px;border-radius:10px;border:1px solid var(--color-border);background:var(--color-surface);cursor:pointer;font-size:12px;font-weight:700;color:var(--color-ink-muted)}
      .em-pill.sel{border-color:var(--color-accent);background:var(--color-accent);color:#fff}
      .em-slider{-webkit-appearance:none;width:100%;height:4px;border-radius:2px;outline:none;cursor:pointer}
      .em-slider::-webkit-slider-thumb{-webkit-appearance:none;width:18px;height:18px;border-radius:50%;background:var(--color-accent);cursor:pointer}
      .em-save{width:100%;padding:14px;background:var(--color-accent);color:white;border:none;border-radius:12px;font-size:13px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;cursor:pointer}
    `}</style>
    <div className="em-section"><div className="em-title">Kimlik</div><div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 16 }}><div onClick={() => fileRef.current?.click()} style={{ width: 64, height: 64, borderRadius: 16, border: '1.5px solid var(--color-border)', background: 'var(--color-surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', overflow: 'hidden' }}>{avatar ? <img src={avatar} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Camera size={20} style={{ color: 'var(--color-ink-muted)' }} />}</div><input ref={fileRef} type="file" accept="image/*" onChange={handleAvatarChange} style={{ display: 'none' }} /><div style={{ flex: 1 }}><span className="em-label">İsim / Mahlas</span><input className="em-input" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="İsmin" /></div></div><div className="em-grid"><div><span className="em-label">Sınav Yılı</span><select className="em-input" value={examYear} onChange={(e) => setExamYear(e.target.value)}><option value="2025">YKS 2025</option><option value="2026">YKS 2026</option><option value="2027">YKS 2027</option></select></div><div><span className="em-label">Alan</span><div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>{TRACKS.map((trackOption) => <button key={trackOption.value} type="button" className={`em-pill ${track === trackOption.value ? 'sel' : ''}`} onClick={() => setTrack(trackOption.value)}>{trackOption.label}</button>)}</div></div></div></div>
      ...
      <div className="em-grid">
        <div style={{ position: 'relative' }}>
          ...
          {showUniDrop && uniSuggestions.length > 0 && (
            <div className="ps-drop" style={{ top: '100%', left: 0, right: 0, backgroundColor: 'var(--color-bg)', border: '1px solid var(--color-border)', color: 'var(--color-ink)' }}>
              {uniSuggestions.map((program) => (
                <div key={program.id} className="ps-drop-item" onMouseDown={() => handleSelectUni(program)}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
                    <div>
                      <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: 'var(--color-ink)' }}>{program.university}</p>
                      <p style={{ margin: '2px 0 0', fontSize: 11, color: 'var(--color-ink-muted)' }}>{program.major}</p>
                    </div>
                  </div>
                </div>
              ))}
    ...
    <div className="em-section"><div className="em-title">Net Hedefleri</div><div style={{ marginBottom: 16 }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}><span className="em-label" style={{ margin: 0 }}>TYT Hedefi</span><span style={{ fontSize: 20, fontWeight: 800, color: '#C17767' }}>{tytTarget}</span></div><input type="range" min={40} max={120} value={tytTarget} onChange={(e) => { const v = Number(e.target.value); if(!isNaN(v)) setTytTarget(v); }} className="em-slider" style={{ background: `linear-gradient(to right,#C17767 0%,#C17767 ${tytPct}%,rgba(255,255,255,.1) ${tytPct}%,rgba(255,255,255,.1) 100%)` }} /></div><div style={{ marginBottom: 16 }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}><span className="em-label" style={{ margin: 0 }}>AYT Hedefi</span><span style={{ fontSize: 20, fontWeight: 800, color: '#E09F3E' }}>{aytTarget}</span></div><input type="range" min={20} max={80} value={aytTarget} onChange={(e) => { const v = Number(e.target.value); if(!isNaN(v)) setAytTarget(v); }} className="em-slider" style={{ background: `linear-gradient(to right,#E09F3E 0%,#E09F3E ${aytPct}%,rgba(255,255,255,.1) ${aytPct}%,rgba(255,255,255,.1) 100%)` }} /></div><div><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}><span className="em-label" style={{ margin: 0 }}>Günlük Min. Saat</span><span style={{ fontSize: 20, fontWeight: 800, color: '#3B82F6' }}>{minHours}</span></div><input type="range" min={1} max={16} value={minHours} onChange={(e) => { const v = Number(e.target.value); if(!isNaN(v)) setMinHours(v); }} className="em-slider" style={{ background: `linear-gradient(to right,#3B82F6 0%,#3B82F6 ${minHoursPct}%,rgba(255,255,255,.1) ${minHoursPct}%,rgba(255,255,255,.1) 100%)` }} /></div></div>
    <div className="em-section"><div className="em-title">Koç Karakteri</div><div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>{COACH_OPTIONS.map((option) => <button key={option.id} type="button" style={{ padding: '12px 16px', borderRadius: 12, border: `1px solid ${coachPersonality === option.id ? option.color : 'var(--color-border)'}`, background: coachPersonality === option.id ? option.glow : 'var(--color-surface)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, width: '100%' }} onClick={() => setCoachPersonality(option.id)}><span style={{ fontSize: 20 }}>{option.icon}</span><div style={{ textAlign: 'left' }}><p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: coachPersonality === option.id ? option.color : 'var(--color-ink)' }}>{option.title}</p><p style={{ margin: '1px 0 0', fontSize: 10, color: 'var(--color-ink-muted)' }}>{option.subtitle}</p></div>{coachPersonality === option.id && <CheckCircle2 size={14} style={{ color: option.color, marginLeft: 'auto' }} />}</button>)}</div></div>
    ...
    <div className="em-section">
      <div className="em-title">Koçun Enerjisi (AI Limit)</div>
      <AiEnergyWidget />
    </div>
...
function AiEnergyWidget() {
  const dailyAiRequests = useAppStore(s => s.dailyAiRequests);
  const maxRequests = 100;
  const percent = Math.min((dailyAiRequests / maxRequests) * 100, 100);
  const energyLeft = Math.max(maxRequests - dailyAiRequests, 0);
  
  return (
    <div style={{ padding: 16, borderRadius: 12, border: '1px solid rgba(193,119,103,.2)', background: 'rgba(193,119,103,.04)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12, alignItems: 'center' }}>
        <div>
          <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: 'white' }}>Mental Kapasite</p>
          <p style={{ margin: '2px 0 0', fontSize: 11, color: 'rgba(255,255,255,.4)' }}>Koçun günlük analiz gücü</p>
        </div>
        <div style={{ textAlign: 'right' }}>
          <p style={{ margin: 0, fontSize: 16, fontWeight: 800, color: percent > 90 ? '#EF4444' : '#C17767' }}>
            {energyLeft} <span style={{ fontSize: 11, fontWeight: 500, color: 'rgba(255,255,255,.4)' }}>kaldı</span>
          </p>
        </div>
      </div>
      <div style={{ height: 6, background: 'rgba(255,255,255,.05)', borderRadius: 3, overflow: 'hidden' }}>
        <div style={{ 
          height: '100%', 
          width: `${100 - percent}%`, 
          background: percent > 90 ? '#EF4444' : percent > 60 ? '#F59E0B' : '#10B981',
