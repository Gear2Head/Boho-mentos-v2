 ...
 * ÇÖZÜM EKRANI: Soru + CanvasLayer + QuestionNav içerir.
 */
    ...
    <div className="fixed inset-0 z-[100] bg-app flex flex-col font-sans overflow-hidden">
      ...
      <header className="h-16 border-b border-[#EAE6DF] dark:border-zinc-800 flex items-center justify-between px-6 bg-app shrink-0 relative z-[110]">
        <div className="flex items-center gap-6">
          <button onClick={quitSession} className="p-3 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full transition-colors text-[#4A443C] dark:text-zinc-400">
            <X size={24} />
          </button>
      ...
      <main className="flex-1 overflow-hidden flex flex-col md:flex-row bg-[#FDFBF7] dark:bg-[#121212]">
         ...
         <div className="flex-1 relative flex flex-col overflow-hidden">
            ...
            <div className="absolute top-4 right-4 z-40 flex gap-2">
              <button 
                onClick={() => store.setDrawingMode(store.drawingMode === 'eraser' ? 'pointer' : 'eraser')}
                className={`px-4 py-2 rounded-xl text-xs font-bold font-mono tracking-widest uppercase transition-all shadow-sm ${store.drawingMode === 'eraser' ? 'bg-[#C17767] text-white scale-105' : 'bg-white dark:bg-zinc-900 text-[#4A443C] dark:text-zinc-300 border border-[#EAE6DF] dark:border-zinc-800'}`}
              >
                Silgi
              </button>
              <button 
                onClick={() => store.setDrawingMode(store.drawingMode === 'pen' ? 'pointer' : 'pen')}
                className={`px-4 py-2 rounded-xl text-xs font-bold font-mono tracking-widest uppercase flex items-center gap-2 transition-all shadow-sm ${store.drawingMode === 'pen' ? 'bg-[#C17767] text-white scale-105' : 'bg-white dark:bg-zinc-900 text-[#4A443C] dark:text-zinc-300 border border-[#EAE6DF] dark:border-zinc-800'}`}
              >
            ...
            {/* Canvas implementation that captures all strokes */}
            <CanvasLayer />

            {/* Question UI Layout (Under the canvas for clicks to pass through except for pointer mode) */}
            <div className="absolute inset-x-0 top-0 bottom-24 p-6 md:p-12 overflow-y-auto pointer-events-none custom-scrollbar">
               <span className="inline-block px-3 py-1 bg-[#C17767]/10 text-[#C17767] text-[10px] uppercase font-bold tracking-widest rounded mb-6">
                 SORU {currentIdx + 1} / {warRoomSession.questions.length} • {question.subject} • {question.topic}
               </span>
               ...
               {question.image && (
                 <img src={question.image} alt="Soru görseli" className="mt-8 rounded-xl border-4 border-white dark:border-zinc-800 shadow-2xl max-w-3xl pointer-events-none" />
               )}
            </div>
         ...
         <aside className="hidden md:flex flex-col w-80 lg:w-96 border-l border-[#EAE6DF] dark:border-zinc-800 bg-[#F5F2EB] dark:bg-zinc-950 p-6 relative z-50">
           <div className="flex items-center gap-3 mb-6">
             ...
             <h3 className="text-xs font-bold tracking-[0.2em] uppercase text-[#4A443C] dark:text-zinc-400">Simülasyon Aktif</h3>
           </div>
           ...
           <div className="bg-white dark:bg-[#121212] rounded-2xl p-6 border border-[#EAE6DF] dark:border-zinc-800 shadow-sm flex-1">
             <div className="space-y-4 text-sm font-mono text-[#4A443C] dark:text-zinc-400">
                <p>Oturum kilitli.</p>
                <p>Soruyu dikkatlice oku, kalemi (sağ üst) seçtiğinde ekran boyunca serbest çizim yapabilirsin.</p>
                <p>Tıklama ve seçenek işaretlemek için "İmleç" moduna dönmelisin.</p>
                <p>İmleç modundayken bir şıkka sağ tıkladığında (veya uzun bastığında) şıkkın üstünü çizersin.</p>
             </div>
           </div>
         </aside>
