...
import { collection, collectionGroup, doc, query, where, getDocs, updateDoc, deleteDoc, writeBatch, getDoc, setDoc, orderBy, limit as firestoreLimit, getCountFromServer } from 'firebase/firestore';
...
export async function fetchSystemAnalytics(): Promise<{
  ...
  try {
    ...
    const q = query(logsRef, where('date', '>=', last24h.slice(0, 10)), firestoreLimit(500));
    const snap = await getDocs(q);
    const activeUsers = new Set(snap.docs.map(d => d.ref.parent.parent?.id).filter(Boolean)).size;
    const totalQuestionsSolved = snap.docs.reduce((sum, d) => sum + (Number(d.data().questions) || 0), 0);
    const trendMap = new Map<string, number>();
    snap.docs.forEach(d => {
      const day = String(d.data().date || '').slice(0, 10);
    ...
    return {
      ...
      logsTrend: [...trendMap.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([date, count]) => ({ date, count })),
    };
  } catch (e) {
    return { totalUsers: 0, activeUsers24h: 0, totalQuestionsSolved: 0, logsTrend: [] };
  }
}
...
export const getEntities = async (table: EntityTable, limitNum?: number) => {
  const snapshot = await getDocs(query(collectionGroup(db, table), firestoreLimit(limitNum || 50)));
