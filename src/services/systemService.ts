...
export async function getSystemConfig(): Promise<SystemConfig | null> {
  const [maintenance, announcement, killSwitch] = await Promise.all([
...
export async function updateApiKeys(
  actorUid: string,
  _apiKeys: { groq?: string; gemini?: string; cerebras?: string }
): Promise<{ success: boolean; error?: string }> {
  await logAdminAction({
    actorUid,
    targetUid: 'SYSTEM',
    action: 'UPDATE_API_KEYS_BLOCKED_CLIENT_STORAGE',
    result: 'failure',
  });
  return { success: false, error: 'API keys are managed through server environment variables only.' };
}
